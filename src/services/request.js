...
service.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 404 && (error.config?.url?.includes('/loans') || error.config?.url?.includes('loan-repayments'))) {
      // silently ignore 404 for not-yet-created loan endpoints
