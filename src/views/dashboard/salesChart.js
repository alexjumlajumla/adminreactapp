...
export default function SalesChart() {
  ...
  const categories = useMemo(
    () => sales.map((item) => {
      // Format with day, month and year to ensure uniqueness
      // Add explicit timezone handling to prevent 3-hour offset
      return moment.utc(item.date).local().format('D MMM YYYY');
    }),
    [sales],
  );
